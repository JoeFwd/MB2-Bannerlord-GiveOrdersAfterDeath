    ...
    private static void Main() {
        ...
        using (var proc = new Process()) {
            proc.StartInfo = new ProcessStartInfo {
                FileName = "git",
                Arguments = "archive --prefix=GiveOrdersAfterDeath/ -o GiveOrdersAfterDeath.zip -9 HEAD",
        ...
        using var fs = File.Open("GiveOrdersAfterDeath.zip", FileMode.Open);
        ...
        foreach (var filePath in Directory.EnumerateFiles(dir)) {
            ...
            var ze = za.CreateEntry("GiveOrdersAfterDeath/bin/Win64_Shipping_Client/" + fi.Name, CompressionLevel.Optimal);
